...
export default function GameRoom() {
  ...
  const [gameState, setGameState] = useState("waiting"); // waiting, shuffling, playing, results, finished
  const [shufflerName, setShufflerName] = useState("");
  ...
  useEffect(() => {
    ...
    socketService.on("can_start_game", handleCanStartGame);
    ...
    return () => {
      ...
      socketService.off("can_start_game", handleCanStartGame);
  ...
  const handleCanStartGame = () => {
    setCanStartGame(true);
    toast.success("All players ready! You can start the game.");
  };

  const handleGameStarted = ({ shufflerUserId, shufflerName: name, round }) => {
    setGameState("shuffling");
    setShufflerName(name);
    setCurrentRound(round);
    toast.success(`Game started! ${name} will shuffle the roles.`);
  };
  ...
  const handleRoleAssigned = ({ role, isSipahi: sipahi }) => {
    ...
    toast.success(`Your role: ${role}`);
  };

  const handleRolesShuffled = ({ round }) => {
    setCurrentRound(round);
    toast.success("Roles have been assigned!");
  };

  const handleGuessResult = ({ sipahiName, guessedName, chorName, isCorrect, pointsDistribution, totalScores, roles }) => {
    setRoundResult({
      sipahiName,
      guessedName,
      chorName,
    ...
    if (isCorrect) {
      toast.success(`${sipahiName} guessed correctly! ${chorName} was the Chor!`);
    } else {
      toast.error(`${sipahiName} guessed wrong! ${guessedName} was not the Chor. ${chorName} was the Chor!`);
    }
  };

  const handleNextRound = ({ round, shufflerUserId, shufflerName: name, totalScores }) => {
    setCurrentRound(round);
    setGameState("shuffling");
    setShufflerName(name);
    setMyRole(null);
    ...
    setRoundResult(null);
    setScores(totalScores);
    toast.success(`Round ${round} starting! ${name} will shuffle.`);
  };
